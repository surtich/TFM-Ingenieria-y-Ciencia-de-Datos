...
En el apéndice (ver @sec-bayes-2) se comparan diversas parametrizaciones de modelado bayesiano utilizando la función `brm()` del paquete `brms`. Analizamos aquí la que mejor resultado produjo con la validación cruzada bayesiana `leave-one-out`:
...
En la @tbl-model-comp se comparan las estimaciones puntuales que obtuvimos para este modelo con la función `clmm` para ajustar el modelo con efectos mixtos. Se comprueba que son muy similares. Vemos que los interceptores son claramente significativos y también el coeficiente de `TreatB`. Sin embargo los coeficientes correspondientes al efecto periodo, `Period2`, y al efecto secuencia, `TreatB:Period`, incluyen el cero y además tienen intervalos muy grandes por lo que hay mucha incertidumbre respecto a su verdadero valor.  
