...
brm_cs.treat.subject.question <- brm(
    Response ~ 1 + cs(Treat) + (1 | Subject) + (1 | Item),
    data = df_clean,
    family = cumulative("logit"),
    sample_prior = TRUE,
...
```{r}
#| eval: false
conditions <- make_conditions(brm_treat.period.subject.question, vars = c("Item"))
...
brm_treat_question_subject_question <- brm(
    Response ~ 1 + Treat + (1 | Item) + (1 | Subject:Item),
    data = df_clean,
    family = cumulative("logit"),
    sample_prior = TRUE,
...
brm_treat_question_subject <- brm(
    Response ~ 1 + Treat + (1 | Item) + (1 | Subject),
    data = df_clean,
    family = cumulative("logit"),
    sample_prior = TRUE,
...
brm_period.treat.subject_question <- brm(
    Response ~ Period * Treat + (1 + Treat | Subject) + (1 | Item),
    data = df_clean,
    family = cumulative("logit"),
    sample_prior = TRUE,
...
brm_period.treat.question_subject <- brm(
    Response ~ Period * Treat + (1 | Subject) + (1 + Treat | Item),
    data = df_clean,
    family = cumulative("logit"),
    sample_prior = TRUE,
...
brm_thres.question_period.treat.subject <- brm(
    bf(Response | thres(gr = Item) ~ Period * Treat + (1 + Treat | Subject)),
    data = df_clean,
    family = cumulative("logit"),
    sample_prior = TRUE,
...
brm_period.treat.subject_question.disc <- brm(
    bf(
        Response ~ Period * Treat + (1 + Treat | Subject) + (1 | q | Item),
        disc ~ 1 + (1 | q | Item)
    ),
    data = df_clean,
    family = cumulative("logit"),
