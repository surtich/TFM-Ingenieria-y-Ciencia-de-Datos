...
```{r}
modelsummary(list(`Response ~ 1` = clm(Response ~ 1, data = df_clean), `Response ~ Treat` = clm(Response ~ Treat, data = df_clean)), estimate = "{estimate}{stars}", gof_omit = "RMSE", statistic = c("std.error", "conf.int"), shape = term:contrast ~ model + statistic, output = "kableExtra", metrics = "Log.Lik.")
```
