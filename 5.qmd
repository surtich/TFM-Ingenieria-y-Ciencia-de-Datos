...
improve_question <- 1 / (1 + exp(-coef(glmer_improve_subject_question)$Item + fixef(glmer_improve_subject_question)))
...
#| tbl-cap: Resumen del modelo Improve ~ 1 + (1 | Subject) + (1 | Item)
modelsummary(list("Improve ~ 1 + (1 | Subject) + (1 | Item)" = glmer_improve_subject_question), gof_omit = "RMSE|R2|ICC", output = "kableExtra")
``` 
...
#| tbl-cap: Probabilidad de que la respuesta a un ítem sea A > B frente a A <= B
 inner_join(
     ...
     data_color(
         rows = Item %in% q.no.errors, columns = Prob, palette = "ggsci::grey_material", target_columns = c(Freq, Item, Prob), domain =
             c(0.1, .7), autocolor_text = T
     ) %>%
     data_color(
         rows = Item %in% q.errors, columns = Prob, palette = "ggsci::purple_material", target_columns = c(Freq, Item, Prob), reverse =
             T, domain = c(0.3, 0.87)
     )
```
...
pred2_brm <- brm_treat.period.subject.item %>%
    epred_draws(ndraws = 50, newdata = expand.grid(list(Period = c(1, 2), Treat = levels(df_clean$Treat), Item = levels(df_clean$Item))), re_formula = ~ (1 + Treat | Item), by = c("Treat", "Item"), category = "Response") %>%
