...
* Fichero `grade` contiene el identificador de estudiante y el grupo al que pertenece (campo `cohort`), ofuscado con \gls{sha}-512 para no conocer la identidad real del estudiante y con \gls{crc}-32b para no conocer si el grupo es el que el vio primero el vídeo bien subtitulado.
