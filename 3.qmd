...
```{r}
str(df_clean %>% dplyr::select(Seq, Period, Treat, Subject, Item, Response, Level), strict.width = "cut")
``` 

En la @tbl-df_clean se muestran algunos ejemplos de datos. Concretamente se muestran las respuestas a tres ítems de dos estudiantes que vieron los vídeos en distinto orden.
