...
compare_test_df %>% ggplot(aes(x = Total, y = n, fill = Total >= 0)) +
    ...
    labs(y = "Cantidad de test (en negativo la diferencia en eje x es negativa)", x = "Diferencias entre test de un estudiante (negativo cuando el segundo test es valorado peor)")
```
...
En la @tbl-resume se muestra la absoluta del valor de respuesta para cada grupo y test en todas la preguntas. 
Esta es otra forma de comparar los niveles de subtitulado. La @fig-freqs muestra la misma información gráficamente y con frecuencias relativas. En esta figura se pueden apreciar algunas cuestiones interesantes:
