...
df %>% ggplot(aes(x = Question, y = Response, color = Treat)) +
...
fit.qt <- aov(Response ~ Treat * Question, data = df)
summary(fit.qt)
```
...
#| echo: true
options(contrasts = c("contr.sum", "contr.poly"))
fit.pq <- aov(Response ~ Period * Question, data = df)
summary(fit.pq)
```
...
```{r}
#| echo: true
fit.tpq <- lm( Response ~ Treat + Period + Question, data = df,
...
Vemos que al haber añadido todas las preguntas ahora **QQ-plot** de los residuos del modelo $Response \sim Question + Treat$ se acerca más a la normalidad.
