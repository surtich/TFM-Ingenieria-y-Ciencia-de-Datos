...
m4 <- brm(Improve ~ 1 + Seq + (1 | Subject) + (1 | Item), family = "bernoulli", data = df_improve)
m4

options(contrasts = c("contr.treatment", "contr.treatment"))
options(contrasts = c("contr.sum", "contr.sum"))
m5 <- brm(Improve ~ 1 + Seq + (1 | Subject) + (1 | Item), family = "bernoulli", data = df_improve)
m5
...
question_effects_hat <- ranef(m5, summary = FALSE)$Item[, , "Intercept"]
...
question_means <- tapply(df_improve$Improve, df_improve[, c("Seq", "Item")], mean)
...
short_hypothesis(m5, "Intercept = 0", scope = "ranef", group = "Item")
...
bmmb::getcorrs(brm_treat.period.subject.question, factor = "Item")
