...
m1 <- lm(as.numeric(Response) ~ Treat, data = df_clean)
m2 <- lm(as.numeric(Response) ~ Treat * Period, data = df_clean)

stargazer(m1, m2)

df <- df_clean %>% mutate(Response = factor(Response == 5))
...
m1 <- clm(Response ~ Treat, data = df_clean)
...
m0 <- clm(Response ~ 1, data = df_clean)
summary(m0)
...
m2 <- clm(Response ~ Treat + Period, data = df_clean)
summary(m2)
...
m3 <- clm(Response ~ Treat + Period + Seq, data = df_clean)
summary(m3)
...
m4 <- clm(Response ~ Period, nominal = ~ Treat + Seq, data = df_clean)
summary(m4)
nominal_test(m4)
m4$Theta

scale_test(m3)
m5 <- clm(Response ~ Treat + Period + Seq, scale = ~ Treat + Seq, data = df_clean)
summary(m5)
convergence(m3)
...
model1 <- vglm(Response ~ Treat, cumulative(parallel = TRUE, reverse = F), data = df_clean)
summary(model1)

model2 <- vglm(Response ~ Treat + Period + Seq, cumulative(parallel = TRUE, reverse = T), data = df_clean)
summary(model2)

apply(predict(model2, type = "response"), 1, which.max)

model2c <- vglm(Response ~ Treat + Period + Seq, cumulative(parallel = F, reverse = T), data = df_clean)
summary(model2c)
...
model3c <- vglm(Response ~ Treat + Period + Seq, cumulative(parallel = F ~ Treat, reverse = T), data = df_clean)
summary(model3c)
...
crmodel2 <- vglm(Response ~ Treat + Period + Seq, sratio(parallel = TRUE, reverse = FALSE), data = df_clean)
summary(crmodel2)

apply(predict(crmodel2, type = "response"), 1, which.max)

crmodel3 <- vglm(Response ~ Treat + Period + Seq, sratio(parallel = F, reverse = FALSE), data = df_clean)
summary(crmodel3)

apply(predict(crmodel3, type = "response"), 1, which.max)

acmodel2 <- vglm(Response ~ Treat + Period + Seq, acat(parallel = TRUE, reverse = FALSE), data = df_clean)
summary(acmodel2)

apply(predict(acmodel2, type = "response"), 1, which.max)


acmodel3 <- vglm(Response ~ Treat + Period + Seq, acat(parallel = F, reverse = FALSE), data = df_clean)
summary(acmodel3)

sum(apply(predict(model3c, type = "response"), 1, which.max) == df_clean$Response)
sum(apply(predict(acmodel3, type = "response"), 1, which.max) == df_clean$Response)


mulmodel2 <- vglm(Response ~ Treat + Period + Seq, multinomial(refLevel = 1), data = df_clean)
summary(mulmodel2)

apply(predict(mulmodel2, type = "response"), 1, which.max)

# pag 759
mpo.1 <- clmm(Response ~ 1 + (1 | Subject), data = df_clean, Hess = TRUE, nAGQ = 7)
summary(mpo.1)

exp(coef(mpo.1))

mpo.2 <- clmm(Response ~ Treat + (1 | Subject), data = df_clean, Hess = TRUE, nAGQ = 7)
summary(mpo.2)
