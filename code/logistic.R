...
m3 <- glmer(Improve ~ 1 + Seq + (1 | Question), data = df_improve)
m4 <- glmer(Improve ~ 1 + Seq + (1 | Question) + (1 | Subject), data = df_improve)
m5 <- glmer(Improve ~ 1 + Seq + (1 + Seq | Subject), data = df_improve)
m6 <- glmer(Improve ~ 1 + Seq + (1 + Seq | Question), data = df_improve)
m7 <- glmer(Improve ~ 1 + Seq + (1 + Seq | Question) + (1 + Seq | Subject), data = df_improve)
...
m10 <- brm(Improve ~ 1 + (1 | Question) + (1 | Subject), family = "bernoulli", data = df_improve)

bmmb::p_check(m10)
short_summary(m10)

fixef(m10)
pp_check(m10, "bars_grouped", group = "Question", ndraws = 1000)


sum((predict(m10)[, 1] > 0.5) == df_improve$Improve) / df_improve %>% nrow()
sum((predict(m10, newdata=df_improve %>% select(Improve, Question), re_formula= ~ (1 | Question))[, 1] > 0.5) == df_improve$Impr   ove) / df_improve %>% nrow()
...
sum(apply(predict(brm_treat.period.subject.question,
    newdata = df_clean %>% select(Treat, Question, Seq, Response, Period),
    re_formula = ~ (1 + Treat | Question), type = "response", ndraws = 100
), 1, which.max) == df_clean$Response) / nrow(df_clean)

df_improve$Prob = predict(m10)[, 1]

 df_improve %>% ggplot(aes(x = Question, y = Prob, fill = Question)) +
     geom_boxplot()

df_improve %>% group_by(Question) %>% summarize(Improve=sum(Improve)/n()) %>% ggplot(aes(x = Question, y = Improve, fill = Ques
    tion)) + 
...
df_improve_resume %>% ggplot(aes(y= Improve)) + geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 0.5, ymax = Inf), fill = "green", alpha = 0.2) +  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax
     = 0.5), fill = "red", alpha = 0.2)  + geom_text(size=8,position = position_jitter(height = 0, width=0.2), aes(x=0, label=Question, y=Improve)) + geom_hline(yintercept = 0.5, linetype = "dashed", size = 2) + labs(y="Frequency of improve A - B", x=NULL)  + 
