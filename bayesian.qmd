...
```{r}
df.improved <- df %>%
    pivot_wider(values_from = Response_v, names_from = Treat, names_prefix = "Treat", id_cols = c(Seq, Subject, Question)) %>%
    filter(!is.na(TreatA) & !is.na(TreatB)) %>%
    mutate(diff = TreatA - TreatB, Improved = as.numeric(diff > 0))
```

```{r}
xtabs(~ Improved + Question, data = df.improved)
```
...
brms::get_prior(Improved ~ Seq + (1|Subject) + (1|Question), data = df.improved)[, -c(7:9)]
...
model.improved.priors <-
    brm(Improved ~ Seq + (1|Subject) + (1|Question),
