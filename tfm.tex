...
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, frame hidden, breakable, boxrule=0pt, enhanced, sharp corners, interior hidden]}{\end{tcolorbox}}\fi
...
en la aproximación bayesiana a validación cruzada
\texttt{leave-one-out}:
...
\toprule
 & \multicolumn{3}{c}{ordinal::clmm} & \multicolumn{4}{c}{brms::brm} \\ 
\cmidrule(lr){2-4} \cmidrule(lr){5-8}
Name & Estimation.clmm & 2.5\% & 97.5\% & Estimation.brm & std.error & conf.low & conf.high \\ 
\midrule
1|2 & -6.75 & -7.45 & -6.05 & -6.71 & 0.37 & -7.43 & -5.98 \\ 
...
No hemos dado valor a las distribuciones de probabilidad a priori con
fiando que los valores por defecto que asigna \texttt{brm} son
adecuados. En el Tabla~\ref{tbl-priors} se muestran las distribuciones a
priori de los parámetros aleatorios del modelo. En la
Figura~\ref{fig-priors} se constata que toman valores razonables y no
informativos.
...
Es importante asegurar que el entrenamiento del a convergido a su
...
\autocite[ \textcite{burkner2019}]{ver}. En la Figura~\ref{fig-trace} se
comprueba que las cadenas de muestrado se mezclan correctamente y no
muestran autocorrelación en ninguno de las parámetros. Por último, en la
Figura~\ref{fig-predictive} se muestra un a comparación de los
histogramas obtenidos de modelo con los intervalos de confianza
marginales de la función predictiva a posteriori del modelo. En la
mayoría de las preguntas, el muestreo reproduce bastante bien el
histograma en casi todas las preguntas excepto en algunas como la
\texttt{Q16} o la \texttt{Q17}.
...
\caption{\label{fig-trace}MCMC trazado del modelo Response
\textasciitilde{} Treat * Period + (1 + Treat \textbar{} Subject) + (1 +
Treat \textbar{} Question).}
