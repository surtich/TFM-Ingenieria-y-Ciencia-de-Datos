...
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[boxrule=0pt, enhanced, sharp corners, frame hidden, interior hidden, borderline west={3pt}{0pt}{shadecolor}, breakable]}{\end{tcolorbox}}\fi
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, colbacktitle=quarto-callout-note-color!10!white, toptitle=1mm, arc=.35mm, colback=white, breakable, toprule=.15mm, leftrule=.75mm, colframe=quarto-callout-note-color-frame, coltitle=black, opacityback=0, bottomtitle=1mm, rightrule=.15mm, titlerule=0mm, opacitybacktitle=0.6, left=2mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Pregunta de investigación}]
...
estudiante contestó a todas los ítems usando únicamente 2 respuestas
...
resultados se pospone a el Capítulo~\ref{sec-resultados}.
...
hace aquí y se pospone la discusión hasta el uso de modelos mixtos con
la regresión ordinal.
...
\item
  El efecto del subtitulado \(B\) es de signo contrario -0.874. Por ello
  aumenta la probabilidad de menor valor de respuesta.
\item
...
Se muestra el resumen del modelo para constatar que los tres
...
estos modelos. Aquí se justificará el interés de este tipo de modelos
aplicado al caso del subtitulado de vídeos. Hay dos variables
susceptibles de ser incorporadas al modelo como efectos aleatorios. En
primer lugar tenemos a los estudiantes. Es evidente que ellos
...
podemos encontrar un ejemplo con esta parametrización.
...
a los estudiantes como efecto aleatorio:
...
donde \(ij\) es la observación \(i\) del estudiante \(j\). Observe que
...
específica para cada estudiante \(U_{0j}\). Para ajustar el modelo, se
va a utilizar la función \texttt{clmm()} del paquete \texttt{Ordinal} ya
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{clmm\_treat.period\_subject }\OtherTok{\textless{}{-}} \FunctionTok{clmm}\NormalTok{(Response }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Treat }\SpecialCharTok{*}\NormalTok{ Period }\SpecialCharTok{+}\NormalTok{ (}\DecValTok{1} \SpecialCharTok{|}\NormalTok{ Subject), }\AttributeTok{data =}\NormalTok{ df\_clean)}
...
mayor diferencia entre las respuestas de los subtitulados \(A\) y \(B\).
...
muestran los valores de los interceptos aleatorios de los estudiantes.
La media de estos interceptos como se espera es cercana a cero (-0.008).
...
\[
\begin{aligned}
Level\ 1: & \text{logit}(P(Response_{ij} \leq k)) = \tau_{kj} - \beta_1 \text{Treat}_{ij} - \beta_{2j} \text{Period}_{ij} - \beta_{3} \text{Treat}_{ij} * \text{Period}_{ij} \\
Level\ 2: & \tau_{kj}  =  \tau_{k} + U_{0j} \\
          & \beta_{2j}  =  \beta_{2} + U_{1j}
\end{aligned}
\]

\normalsize

Ahora el parámetro \(\beta_{2j}\) del subtitulado tiene dos componentes:
Uno común a todos los estudiantes, \(\beta_{2}\) y otro particular de
cada estudiante, \(U_{1j}\). El modelo ajustado ocasiona que ahora sólo
...
Se comprueba que en este test resulta el último modelo:
