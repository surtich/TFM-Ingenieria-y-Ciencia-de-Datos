...
\def\tablename{Tabla}}
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[frame hidden, interior hidden, borderline west={3pt}{0pt}{shadecolor}, boxrule=0pt, breakable, enhanced, sharp corners]}{\end{tcolorbox}}\fi
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Pregunta de investigación}, colback=white, leftrule=.75mm, titlerule=0mm, toprule=.15mm, left=2mm, rightrule=.15mm, breakable, toptitle=1mm, opacitybacktitle=0.6, bottomtitle=1mm, colbacktitle=quarto-callout-note-color!10!white, arc=.35mm, bottomrule=.15mm, colframe=quarto-callout-note-color-frame, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Objetivos específicos}, colback=white, leftrule=.75mm, titlerule=0mm, toprule=.15mm, left=2mm, rightrule=.15mm, breakable, toptitle=1mm, opacitybacktitle=0.6, bottomtitle=1mm, colbacktitle=quarto-callout-tip-color!10!white, arc=.35mm, bottomrule=.15mm, colframe=quarto-callout-tip-color-frame, opacityback=0]
...
\hypertarget{sec-glm}{%
\section[Modelo Lineales Generalizad]{\texorpdfstring{Modelo Lineales
Generalizad\footnote{No se deben confundir los modelos lineales
  ...
  admita otras distribuciones además de la normal.}}{Modelo Lineales Generalizad}}\label{sec-glm}}
...
\item
  Se seleccionaron las preguntas que contienen las respuestas y se
  renombraron para que fuera más fácil saber de qué pregunta se trataba
  \footnote{En los ficheros suministrados la respuesta a cada pregunta
    ocupaba varios campos. Se seleccionó en cada pregunta el que
    contiene el valor de la respuesta y se convirtió a numérico.}. Se
  convirtió el campo \texttt{LastTry}, que contiene la fecha y hora de
  realización del test, a formato fecha y hora.
\item
...
\item
  ...
  los test.
\item
  Se transformaron las variables que lo requirieron en factores. La
  pregunta 18 se fijó como referencia en el factor \texttt{Question} ya
  que es una valoración general del subtitulado.
\item
...
\item
  ...
  respuestas a cada pregunta. Los nombres de las columnas son \(Q01\),
  ...
  y \(Response\). En la primera se almacena el identificador de la
  pregunta (\(Q01\), \(Q02\), \ldots, \(Q18\)) y en la segunda el valor
  de la respuesta (de 0 a 5). De esta forma, un test pasó de ocupar una
...
valor de respuesta en todas las preguntas.
...
entre 11 y 17 respuestas. Tan solo 1 estudiante respondió a todas las
preguntas con el mismo valor en los dos test. Por otro lado, no hay test
que tengan un número excesivo de contestaciones \enquote{No sé/No
contesto} (ver Tabla~\ref{tbl-noanswer}).
...
de respuesta para cada grupo y test en todas la preguntas. Esta es otra
...
valorado por los estudiantes, pero ahora se confirma que en las 18
preguntas ambos grupos tienen más puntuaciones positivas y menos
negativas en el subtitulado \(A\) que en el \(B\). También se vuelve a
constatar que los dos grupos valoran de forma muy similar los dos
niveles de subtitulado en todas las preguntas. En el nivel de
subtitulado \(A\) las preguntas \(Q15\), \(Q16\) y \(Q17\) obtienen
relativamente peores valoraciones (consultar la
Tabla~\ref{tbl-likert-scale} para ver el texto de las preguntas) y estas
...
(por ejemplo \(Q04\) o \(Q13\)). Por último, las preguntas \(Q05\) y
...
posibles: 1 cuando el estudiante valoró la pregunta mejor en el
subtitulado \(A\) que en el \(B\), 0 si empeoró o puntuó igual. Si en
uno de los test contestó una pregunta con \enquote{No sé / No contesto},
se elimina esa pregunta.
...
probabilidad de que la respuesta a una pregunta sea mejor en el
subtitulado \(A\) que en el \(B\) es 0.59. Sin embargo, la secuencia no
resulta significativa y además añadirla apenas reduce la
\enquote{deviance}, por lo que el modelo nulo sin predictores resulta
más parsimonioso.
...
son moderadas para la mayoría de las preguntas. La diferencia es mayor
en el subtitulado en la comparación de los niveles de respuesta 1 y 2.
Con esta evidencia, se acepta la hipótesis de proporcionalidad de
\(odds\). En la Tabla~\ref{tbl-po.check} se muestra cómo se realiza el
cálculo de la diferencia de \(odds\) para el predictor \texttt{Seq} y
así facilitar la comprensión de la construcción de la figura. En caso de
que la proporcinalidad de \(odds\) no se cumpla existen varias
posibilidades. Una sería desechar el modelo ordinal y usar una Regresión
Multinomial. Otra sería relajar la hipótesis de proporcionalidad de
\(odds\) estimando un coeficiente distinto para cada nivel de respuesta
y nivel de factor. La función \texttt{vglm} del paquete \texttt{VGAM}
permite hacer esto.
...
periodos, secuencias, preguntas o estudiantes. En las siguientes
secciones se investiga si en el nivel de respuesta influyen estos
predictores.
...
Por otro lado, las preguntas no son independientes unas de otras ya que
...
la función predictiva a posteriori del modelo. En la mayoría de las
preguntas, el muestreo se ajusta bastante bien al histograma de
respuestas; aunque en algunas preguntas, como la \texttt{Q16} o la
\texttt{Q17}, se aprecian diferencias relevantes.
...
las preguntas 4 y 5 (ver Tabla~\ref{tbl-logor2}).
...
pequeña corrección en la valoración del subtitulado y que
...
4. En cuanto al análisis individualizado por pregunta se llega a las
