...
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[interior hidden, frame hidden, borderline west={3pt}{0pt}{shadecolor}, sharp corners, boxrule=0pt, breakable, enhanced]}{\end{tcolorbox}}\fi
...
\item
  ...
  particular esto sucederá si hay hay muchas respuestas en los extremos
  del cuestionario.
\item
...
Según \textcite{burkner2019} hay tres clases de regresión ordinal:
...
{\centering \includegraphics{42-Modelado_files/figure-pdf/fig-cumulative-1.pdf}
...
Otra popular elección, que usaremos en este trabajo por ser más fácil su
interpretación, es suponer que la función acumulada se comporta como una
logística. En ese caso, la interpretación de los coeficientes varía y se
asemeja a la de la regresión logística. Se parte del supuesto de que el
\(logit\) de la función de probabilidad es lineal:
...
\autocite[ver][]{Liu2202}.

Un valor del coeficiente \(\beta_i\) positivo indica que la relación
entre el predictor \(x_i\) y la función de \(logit\) es positiva y, por
lo tanto, se incrementa la posibilidad de un mayor valor de la variable
respuesta.

Esta suposición se puede relajar y permitir que los coeficientes de
todos o de algunos de los predictores sean diferentes para cada pareja
consecutiva de valores de respuesta. Tendríamos entonces más parámetros
a estimar con una interpretación más compleja.

\hypertarget{ajuste-del-modelo-ordinal-response-treat.}{%
\subsection{Ajuste del modelo Ordinal Response \textasciitilde{}
Treat.}\label{ajuste-del-modelo-ordinal-response-treat.}}
...
del paquete \texttt{MASS} \autocite{MASSR}. En este trabajo usaremos el
paquete Ordinal por permitir también incluir efectos aleatorios que
...
subtitulado \(A\) que en el \(B\). Aunque no suele ser de interés
podemos usar los coeficientes de los umbrales
(\texttt{Threshold\ coefficients}) para hacer una interpretación en base
a probabilidades de respuesta. Por ejemplo, para el nivel de subtitulado
\(B\) la:
...
\subsection{Ajuste del modelo Ordinal Response \textasciitilde{} Treat +
Period.}\label{ajuste-del-modelo-ordinal-response-treat-period.}}
...
\subsection{Ajuste del modelo Ordinal Response \textasciitilde{} Treat *
Period.}\label{ajuste-del-modelo-ordinal-response-treat-period.-1}}
...
Sin embargo los coeficientes son diferentes si el contraste es
\texttt{treatment} \footnote{Los interceptores sí son iguales.}:
...
En definitiva, los cuatro modelos anteriores son equivalentes y, como se
demuestra en Apéndice~\ref{sec-contrasts}, se pueden obtener los
coeficientes del modelo \texttt{clm\_treat.period} a partir de los
coeficientes del modelo \texttt{clm\_treat.period.sum}. Por ejemplo, el
coeficiente \(TreatB\) del modelo \texttt{clm\_treat.period}:
...
modelo \texttt{clm\_treat.period}.
...
es el nivel de subtitulado obteniendo mejores puntuaciones el nievel
...
{\centering \includegraphics{42-Modelado_files/figure-pdf/fig-probs-clm-treat.period-1.pdf}
...
razón de verosimilitud. Comprobamos que el tercer modelo (el que tiene
...
que hace es realizar un test para cada predictor ajustando un modelo en
el que se ha relajado la condición de proporcionalidad y realiza el test
de razón de verosimilitud de entre este modelo y el modelo que se quiere
evaluar. Se constata que el test resulta significativo para
\texttt{Treat} y para \texttt{Treat:Period}, por lo que para estas dos
variables no se puede asumir que los coeficientes estimados se mantengas
...
{\centering \includegraphics{42-Modelado_files/figure-pdf/fig-probs-vglm-treat.period-1.pdf}
