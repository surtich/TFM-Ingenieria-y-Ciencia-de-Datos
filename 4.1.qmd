...
Se constata que el coeficiente del intercepto es positivo y significativo (`r round(coef(glm_improve_seq)[1],2)`). El intercepto es el $log\ odds$ de mejorar la valoración en $A$ sobre $B$ respecto a empeorar la valoración. La probabilidad de que la respuesta a un ítem sea mejor en el subtitulado $A$ que en el $B$ es `r round(plogis(coef(glm_improve_seq)[1]),2)`. Sin embargo, la secuencia no resulta significativa y además añadirla apenas reduce la "deviance", por lo que el modelo nulo sin predictores resulta más parsimonioso.
