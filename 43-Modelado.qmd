...
donde $Response_{ij}$ es la observación $i$ del usuario $j$, $\tau_k$ es el interceptor común a todos los usuarios y para el nivel de respuesta $k$ y $\tau_{kj}$ es el interceptor específico para el usuario $j$.

 Para ajustar el modelo, vamos a utilizar la función `clmm()` del paquete `Ordinal` ya que permite la inclusión de efectos aleatorios.
