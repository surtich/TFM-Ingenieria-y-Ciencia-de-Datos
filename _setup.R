...
# Eliminamos los test anómalos
test_df <- test_all_df %>% filter(!User %in% c(1020, 85))
...
df <- test_df %>%
    mutate(
        ...
        Treat = as.factor(if_else(Group == "A" & Test == "01" | Group == "B" & Test == "02", "A", "B")),
        Seq = as.factor(if_else(Group == "A", "AB", "BA")), Subject = as.factor(User)
    ) %>%
...
df <- inner_join(df, data.frame(Question = factor(names(cuts), levels = levels(df$Question)), Cluster = as.factor(cuts)), by = "Question")
