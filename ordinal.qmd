...
$$
\begin{aligned}
\frac{P(Y \le 1 | x_2 = B)}{P(Y \gt 1 | x_2 = B)} & = & exp(`r intercept.1.2.coef`)/exp(`r treatB.coef`) \\
\frac{P(Y \le 1 | x_2 = A)}{P(Y \gt 1 | x_2 = A)} & = & exp(`r intercept.1.2.coef`) \\
\frac{P(Y \le 2 | x_2 = B)}{P(Y \gt 2 | x_2 = B)} & = & exp(`r intercept.2.3.coef`)/exp(`r treatB.coef`) \\
\frac{P(Y \le 2 | x_2 = A)}{P(Y \gt 2 | x_2 = A)} & = & exp(`r intercept.2.3.coef`)
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{P(Y \le 1 | x_2=B)}{P(Y \gt 1 | x_2=B)} / \frac{P(Y \le 1 | x_2=A)}{P(Y \gt 1 | x_2=A)} & = & 1/exp(`r treatB.coef`) & = & `r round(1/exp(coef(clm_likert_treat)["TreatB"]), 2)` \\
\frac{P(Y \le 2 | x_2=B)}{P(Y \gt 2 | x_2=B)} / \frac{P(Y \le 2 | x_2=A)}{P(Y \gt 2 | x_2=A)} & = & 1/exp(`r treatB.coef`) & = & `r round(1/exp(coef(clm_likert_treat)["TreatB"]), 2)` \\
\end{aligned}
$$
...
Vemos que la mayoría de los factores `Question` no son significativos.
...
df <- inner_join(df, data.frame(Question = names(cuts), Cluster = cuts), by = "Question")
```
